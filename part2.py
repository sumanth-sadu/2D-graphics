...
class objectGeneration:
    ...
    def colorFace(self, points, figure_points):
        '''
            Method to color each face of 3D object and make the color smoothly vary between #00005F and #0000FF
            To find othogonal vector to each face of 3D object we find cross product of two vectors on that surface. Now find angle (using dot product) between unit vector of z-axis and unit vector
            vector of orthogonal to each face. 
            Now shift/change color of each face based on angle made by orthogonal of that face with z-axis.
        '''

        for surface in self.surfaces:
            v1 = [figure_points[surface[1]][0] - figure_points[surface[0]][0], figure_points[surface[1]][1] - figure_points[surface[0]][1], figure_points[surface[1]][2] - figure_points[surface[0]][2]]
            v2 = [figure_points[surface[2]][0] - figure_points[surface[1]][0], figure_points[surface[2]][1] - figure_points[surface[1]][1], figure_points[surface[2]][2] - figure_points[surface[1]][2]]

            v_orth = np.cross(v1,v2)

            v_orth_unit = v_orth / (sqrt(v_orth[0]*v_orth[0] + v_orth[1]*v_orth[1] + v_orth[2]*v_orth[2]))
            zv = [0, 0, 1]

            theta = acos((np.dot(zv,v_orth_unit)))
            degree = int(theta*57.2958)

            shift = (95 + int((self.COLOR_PER_ANGLE*(degree%90))))

            pygame.gfxdraw.filled_polygon(self.WINDOW, [points[surface[0]], points[surface[1]], points[surface[2]]], (0, 0 ,shift))
    ...
    def mainfun(self, figure_matrix, figure_points):
        ...
        while True:
            ...
            self.colorFace(points, figure_points)
